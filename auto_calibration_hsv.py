...
calibration_points = []
calibration_points.append(Point(0.22, 0,     calibration_points_z))
calibration_points.append(Point(0.22, 0.1,  calibration_points_z))
calibration_points.append(Point(0.22, -0.1,  calibration_points_z))
calibration_points.append(Point(0.16, 0, calibration_points_z))
calibration_points.append(Point(0.28, 0,     calibration_points_z))

# 定义拍照位姿
capture_point = Point(0.15, -0.00, 0.2)
...
for point in calibration_points:
    ...
    # 等待用户确认
    input_value = input("请将红色标定物放置到夹爪正下方，按任意键确认！")
    rospy.sleep(1)
    ...
    if len(regObjList) == 1:        
        # 像素坐标
        xc_array[success_count] = regObjList[0].position.x
        yc_array[success_count] = regObjList[0].position.y
        # 空间坐标
...
if success_count < 3:
    ...
    rospy.loginfo("Linear Regression for x and yc is :  x = %.5fyc + (%.5f)" % (k1, b1))
    rospy.loginfo("Linear Regression for y and xc is :  y = %.5fxc + (%.5f)" % (k2, b2))
