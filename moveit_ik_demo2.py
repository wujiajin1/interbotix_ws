...
class MoveItFkDemo:
    ...
    # Set pick position 
            ...
            arm.go()
            rospy.sleep(2)
        def control(x,y,trash_joint):
            xw = y*-0.0004142410392109696+0.3194178494106327
            yw = x*-0.00042150118447168295+0.13597628211056492
            print("像素坐标：",end=' ')
            print(x,y)
            print("world坐标：",end=' ')
            print(xw,yw)
            ...
            gripper.go()
            rospy.sleep(1)
            arm.set_joint_value_target(trash_joint)
            arm.go()
            rospy.sleep(1)
...
# 标定参数
        ...
        if sys.argv[1] == '0' and len(response.blueObjList)==1:
            print("识别到肠胃炎药，正在为您拿肠胃炎药")
        ...
        if sys.argv[1] == '1' and len(response.redObjList)==1:
            print("识别到感冒药，正在为您拿感冒药")
        ...
        if sys.argv[1] == '2' and len(response.greenObjList)==1:
            print("识别到退烧药，正在为您拿退烧药")
